...
uint8_t SX1278::setSpreadingFactor(uint8_t sf) {
  uint8_t state = SX1278::config(_freq, _bw, sf, _cr, _syncWord);
  if(state == ERR_NONE) {
    _sf = sf;
  }
  return(state);
...
uint8_t SX1278::setCodingRate(uint8_t cr) {
  uint8_t state = SX1278::config(_freq, _bw, _sf, cr, _syncWord);
  if(state == ERR_NONE) {
    _cr = cr;
  }
  return(state);
...
uint8_t SX1278::setFrequency(float freq) {
  uint8_t state = SX1278::config(freq, _bw, _sf, _cr, _syncWord);
  if(state == ERR_NONE) {
    _freq = freq;
  }
  return(state);
